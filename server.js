...
server.get("/", (req, res) => {
    db.select("*")
        ...
        .catch(err => res.status(500).send({message: "Accounts were not able to be found.", error: err}));
});
server.post("/", (req, res) => {
    db("accounts")
        .insert(req.body)
        .then(account =>
            res.status(201).send({response: account, message: "Account successfully added." })
        )
